...
import (
	...
	"github.com/cheelim1/myBot/pkg/config"
	"github.com/cheelim1/myBot/pkg/jokes"
	"github.com/cheelim1/myBot/pkg/password_generator"
...
func handleAppMentionEvent(event *slackevents.AppMentionEvent, client *slack.Client) error {
	...
	} else if strings.Contains(text, "joke"){
		// Respond with random joke to the user
		setup,joke := jokes.JokeOfTheDay()
		attachment.Pretext = fmt.Sprintf("Hi %s 👋 the joke is >> ( %s ) ",user.Name,setup)
		attachment.Text = fmt.Sprintf("🤣 %s 🤣",joke)
		attachment.Color = "#ADD8E6"
	} else if strings.Contains(text, "password"){
		// Respond with random password generated to the user
		randomPassword := password_generator.PasswordGenerator()
		attachment.Pretext = fmt.Sprintf("Hi %s 👋 ",user.Name)
		attachment.Text = fmt.Sprintf("Generated Password: %s",randomPassword)
...
func SlackInit(){
	botToken := config.GoDotEnvVariable("SLACK_BOT_TOKEN")
	_ = botToken
